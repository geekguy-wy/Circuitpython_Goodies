...
display = Seg14x4(i2c, auto_write=True)
display.brightness = DEFAULT_DISPLAY_BRIGHTNESS
print()
print("Display brightness = {0:1.2f}".format(display.brightness))

def animate(digits, bitmasks, delay=DEFAULT_CHAR_DELAY_SEC, auto_write=False):
    '''
...
def chase_forward_and_reverse(delay=DEFAULT_CHAR_DELAY_SEC, cycles=DEFAULT_CYCLES):
    ...
    while cy < cycles:
        animate([0, 1, 2, 3], [A, 0], delay, auto_write)
        animate([3], [B, C, D, 0], delay, auto_write)
        animate([2, 1, 0], [D, 0], delay, auto_write)
        animate([0], [E, F, H, G2, 0], delay, auto_write)
        animate([1, 2], [G1, G2, 0], delay, auto_write)
        animate([3], [G1, J, A, 0], delay, auto_write)
        animate([2, 1], [A, 0], delay, auto_write)
        animate([0], [A, F, E, D, 0], delay, auto_write)
        animate([1, 2], [D, 0], delay, auto_write)
        animate([3], [D, C, B, J, G1, 0], delay, auto_write)
        animate([2, 1], [G2, G1, 0], delay, auto_write)
        animate([0], [H, 0], delay, auto_write)
...
def enclosed_spinners(delay=DEFAULT_CHAR_DELAY_SEC, cycles=DEFAULT_CYCLES):
    ...
    while cy < cycles:
        animate([0], [A+D+E+F+H+M], 0)
        animate([1], [A+D+J+K], 0)
        animate([2], [A+D+H+M], 0)
        animate([3], [A+B+C+D+J+K], 0)
        display.show()
        sleep(delay)

        animate([0], [A+D+E+F+G1+G2], 0)
        animate([1], [A+D+G1+G2], 0)
        animate([2], [A+D+G1+G2], 0)
        animate([3], [A+B+C+D+G1+G2], 0)
        display.show()
        sleep(delay)

        animate([0], [A+D+E+F+J+K], 0)
        animate([1], [A+D+H+M], 0)
        animate([2], [A+D+J+K], 0)
        animate([3], [A+B+C+D+H+M], 0)
        display.show()
        sleep(delay)
...
def warning_arrows(delay=DEFAULT_CHAR_DELAY_SEC, cycles=DEFAULT_CYCLES):
    cy = 0
...
def count_down():
    numbers = [ [A+B+C+D+G1+G2], [A+B+D+E+G1+G2], [B+C] ]
    index = 0

    display.fill(0)

    while index < len(numbers):
        animate([index], numbers[index], 0)
        display.show()
        sleep(1)
        display.fill(0)
        sleep(0.5)
...
try:
    ...
    while True:
        ...
        while cyc < DEFAULT_CYCLES:
            animate([0], [H+J, G1+G2, K+M, G1+G2], 0.1, auto_write)
            display.show()
        ...
        #	Warning Arrows
        print("Warning Arrows")
        warning_arrows(0.35, 5)
        sleep(1.0)
        display.fill(0)
        sleep(2.0)
        ...
        chase_forward_and_reverse(0.02, 5)
        sleep(1.0)
        display.fill(0)
        sleep(2.0)
